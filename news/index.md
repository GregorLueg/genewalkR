# Changelog

## genewalkR 0.1.10

### Features

- Rust version updates
- Nicer website and updades to `README.md` to point to R-universe for
  install.

## genewalkR 0.1.9

### Features

- Rust version updates

## genewalkR 0.1.8

### Features

- New data release
- Various updates to Rust crates to be up-to-date

## genewalkR 0.1.7

### Features

- Vignettes and documentations

## genewalkR 0.1.6

### Features

- Various version bumps

## genewalkR 0.1.5

Shifting versioning to `x.x.x` pattern.

### Features

- Various diffusion methods for “guilty-by-association” methods
  implemented.

## genewalkR 0.0.1.4

### Features

- Gene drift implemented

## genewalkR 0.0.1.3

### Features

- Added functionality to redownload the DB.
- Added Pathway Commons to the DuckDB
- Aligned the statistics of GeneWalk with the original paper and only
  keeping the largest connected component.

## genewalkR 0.0.1.2

### Features

- Moved DB from git lfs to package version.

## genewalkR 0.0.1.1

### Features

- Added plotting functions and removed unnecessary dependencies in
  CI/CD.

## genewalkR 0.0.1.0

### Features

- Package release.
